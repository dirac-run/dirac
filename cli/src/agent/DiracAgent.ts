...
import {
	...
	getLatestTaskIdForSession,
	recordTaskForSession,
} from "../acp/acp-session-tasks.js";
import {
...
function workspaceCheckpointsFromMessages(
...
function worktreeProvisioningRequest(
	params: acp.NewSessionRequest,
): WorktreeProvisioningRequest | undefined {
	...
	if (!requested || typeof requested !== "object" || Array.isArray(requested)) {
		throw new Error(
			"dev.dirac/worktree must be true or an object with an optional baseBranch",
		);
	}
...
export class DiracAgent implements acp.Agent {
	...
	private async releaseSessionResources(sessionId: string): Promise<void> {
		...
		if (
			this.sessionStates.get(sessionId)?.status === AcpSessionStatus.Processing
		) {
	...
	private async deleteSessionResources(sessionId: string): Promise<void> {
		...
		if (worktree) {
			const removal = await deleteWorktree(
				worktree.sourceCwd,
	...
	private async provisionSessionWorktree(
		...
		if (!sourceCwd) {
			throw new Error(
				"dev.dirac/worktree requires cwd to be inside a git repository",
			);
		}
		...
		const targetBranch =
			request.baseBranch ??
			(checkedOutBranch === "HEAD" ? undefined : checkedOutBranch);
		...
		const worktreePath = path.join(
			worktreeDirectory,
			`${path.basename(sourceCwd)}-${sessionId}`,
		);
	...
	async integrateSessionWorktree(
		...
		const activeController = activeSession
			? this.#sessionControllers.get(activeSession)
			: undefined;
		if (activeController?.task) {
			throw new Error(
				`Cannot integrate ACP session ${sessionId} while its task is active; close the session first`,
			);
		}
		...
		if (!branch) {
			throw new Error(
				"targetBranch is required when the session was created from a detached HEAD",
			);
		}

		const targetGit = simpleGit(worktree.sourceCwd);
		const checkedOutBranch = (
			await targetGit.revparse(["--abbrev-ref", "HEAD"])
		).trim();
		if (checkedOutBranch !== branch) {
			throw new Error(
				`Target branch ${branch} is not checked out at ${worktree.sourceCwd}`,
			);
		}
		...
		if (!(await worktreeGit.status()).isClean()) {
			throw new Error(
				"Session worktree has uncommitted changes; commit or stash them before integrating",
			);
		}
	...
	private readonly sessionEmitters: Map<string, DiracSessionEmitter> =
		new Map();
	...
	private createTaskMessageBridge(): TaskMessageBridge {
		return new TaskMessageBridge({
			...
			getController: (session: DiracAcpSession) =>
				this.#sessionControllers.get(session),
			requestPermission: (sessionId, toolCall, options) =>
				this.requestPermission(sessionId, toolCall, options),
			emitSessionUpdate: (sessionId, update) =>
				this.emitSessionUpdate(sessionId, update),
			emitUsageUpdate: (sessionId, usage) =>
				this.emitUsageUpdate(sessionId, usage),
			persistPermissionRule: (sessionId, toolCall, action) =>
				this.persistPermissionRule(sessionId, toolCall, action),
	...
	async queueWhisper(params: Record<string, unknown>): Promise<void> {
		...
		if (
			...
			Logger.debug(
				"[DiracAgent] Ignoring malformed dev.dirac/whisper notification",
			);
			return;
		}
		if (
			...
			Logger.debug(
				"[DiracAgent] Ignoring whisper outside an active turn:",
				sessionId,
			);
			return;
		}
	...
	recordClientAnnotation(params: Record<string, unknown>): void {
		...
		if (
			...
			Logger.debug(
				"[DiracAgent] Ignoring malformed dev.dirac/client_annotation notification",
			);
			return;
		}
	...
	async pinMessage(sessionId: string, messageId: string): Promise<void> {
		...
		const controller =
			this.#sessionControllers.get(session) ??
			(session as DiracAcpSession & { controller?: Controller }).controller;
		const task = controller?.task;
		const message = task?.messageStateHandler
			.getDiracMessages()
	...
	async unpinMessage(sessionId: string, messageId: string): Promise<void> {
		const session = this.sessions.get(sessionId);
		if (!session) throw new Error(`Unknown session: ${sessionId}`);
		if (!unpinSessionMessage(sessionId, messageId))
			throw new Error(`Message is not pinned: ${messageId}`);
	...
	private pinnedContentFromMessage(message: DiracMessage): string | undefined {
		if (message.content.type === DiracMessageType.MARKDOWN)
			return message.content.content || undefined;
		if (message.content.type === DiracMessageType.CARD)
			return message.content.card.body || message.content.card.header;
		return undefined;
	}
	...
	private applyPinnedContext(
		...
		task.setContextCompactionObserver(
			() => void this.emitPinnedMessagesUpdate(sessionId, "compacted"),
		);
	}
	private async emitPinnedMessagesUpdate(
		sessionId: string,
		event: "pinned" | "unpinned" | "compacted",
	): Promise<void> {
	...
	async listWorkspaceCheckpoints(
		...
		const task = controller.task;
		if (task) {
			return workspaceCheckpointsFromMessages(
				task.messageStateHandler.getDiracMessages(),
			);
		}

		const taskId =
			session.loadedTaskId ?? getLatestTaskIdForSession(sessionId) ?? sessionId;
		const { uiMessagesFilePath } = await controller.getTaskWithId(taskId);
		const messages: DiracMessage[] = JSON.parse(
			await fs.readFile(uiMessagesFilePath, "utf8"),
		);
	...
	private readonly pendingPermissionResolvers: Map<
		string,
	...
	async listPermissionRules(sessionId: string): Promise<ToolPermissionRule[]> {
		...
		return await this.withSessionPermissionController(sessionId, (controller) =>
	...
	async deletePermissionRule(
		...
		await this.withSessionPermissionController(sessionId, (controller) =>
	...
	private permissionTaskForSession(sessionId: string) {
		...
		if (!task) {
			throw new Error(
				"Cannot persist a permission rule without an active session task",
			);
		}
		return task;
	}

	private permissionRuleToolName(
		toolCall: acp.ToolCall | acp.ToolCallUpdate,
	): string {
		if (!toolCall.title) {
	...
	private async requestPermission(
		...
		return new Promise((resolve) => {
			...
			this.permissionHandler!(
				{ sessionId, toolCall, options: options || [] },
				settle,
			);
		});
	}
			...
			authMethods: [
				{
					...
					description:
						"Authenticate with your ChatGPT Plus/Pro/Team subscription",
				},
			],
		};
	}
	...
	async newSession(
		...
		const worktree = worktreeRequest
			? await this.provisionSessionWorktree(
				sessionId,
				params.cwd,
				worktreeRequest,
			)
		...
		return {
			sessionId,
			modes: this.sessionConfig.getSessionModeState(
				session.mode,
	...
	async loadSession(
		...
		const historyCwd = ownedWorktree?.worktreePath ?? getHistoryItemCwd(
			persistedHistory,
			params.cwd,
			this.options.cwd,
		);
	...
	async publishSessionSetupUpdates(sessionId: string): Promise<void> {
		...
		if (!controller) {
			throw new Error(
				"Controller not initialized for session. This is a bug in the ACP agent setup.",
			);
		}
	...
	async prompt(params: acp.PromptRequest): Promise<acp.PromptResponse> {
		...
		if (
			(sessionState.status as AcpSessionStatus) === AcpSessionStatus.Cancelled
		) {
			releasePrompt();
		...
		const sessionOverridesToApply =
			this.acpSessionOverrides.get(params.sessionId) ?? {};
		const savedStateManagerOverrides = swapSessionOverrides(
			sessionOverridesToApply,
		);
		...
		try {
			...
			if (isLoadedSession && !hasActiveTask) {
						...
						const onChanged = (change: DiracMessageChange) => {
							if (
								...
								(change.message.content.card.header === "Resume Task" ||
									change.message.content.card.header ===
				...
				Logger.debug(
					"[DiracAgent] Continuing existing task:",
	...
	async cancel(params: acp.CancelNotification): Promise<void> {
		...
		if (!session) {
			Logger.debug(
				"[DiracAgent] cancel called for non-existent session:",
				params.sessionId,
			);
			return;
		}
		...
		if (sessionState) {
			...
			await this.bridgeForSession(params.sessionId).cancelInFlightToolCalls(
				params.sessionId,
				sessionState,
			);
	...
	async setSessionMode(
		...
		if (!validModes.includes(params.modeId as AcpModeId)) {
			throw new Error(
				`Invalid mode: ${params.modeId}. Valid modes are: ${validModes.join(", ")}`,
			);
		}
	...
	private publishSelfInitiatedModeChange(): void {
		...
		this.emitCurrentModeUpdate(sessionId).catch((error) => {
			Logger.error(
				"[DiracAgent] Error publishing self-initiated mode change:",
				error,
			);
		});
		this.emitConfigOptionsUpdate(sessionId).catch((error) => {
			Logger.error(
				"[DiracAgent] Error publishing self-initiated config update:",
				error,
			);
		});
	}
	...
	private async emitCurrentModeUpdate(sessionId: string): Promise<void> {
		...
		await this.emitSessionUpdate(sessionId, {
			...
			currentModeId: this.sessionConfig.computeCurrentAcpModeId(
				session.mode,
				this.acpSessionOverrides.get(sessionId),
			),
		});
	...
	private emitUsageFromApiStatus(
		sessionId: string,
	...
	private emitUsageUpdate(
		...
		this.emitterForSession(sessionId).emit(
			"usage_update",
			recordUsageUpdate(sessionId, usage),
		);
	}
	...
	async replayLoadedSessionHistory(sessionId: string): Promise<void> {
		...
		if (persistedUpdates.length > 0) {
			...
			for (const persistedUpdate of persistedUpdates) {
				if (persistedUpdate.kind === "session_update") {
					emitter.emit(
						persistedUpdate.update.sessionUpdate,
		...
		try {
			...
			Logger.debug(
				"[DiracAgent] replayLoadedSessionHistory: could not read ui_messages:",
				error,
			);
			return;
		}
		...
		for (const message of uiMessages) {
			try {
				...
				Logger.debug(
					"[DiracAgent] replayLoadedSessionHistory: error translating message:",
					error,
				);
			}
		}
	...
	private planFromMessage(message: DiracMessage): acp.Plan | undefined {
		...
		const numberedItems = planText
			...
			.filter(Boolean);
		const planItems =
			numberedItems.length > 0 ? numberedItems : planText ? [planText] : [];
	...
	private async sendAvailableCommands(
		...
		try {
			...
			const filteredCommands = response.commands.filter(
				(cmd) =>
					cmd.cliCompatible &&
					!cliOnlyNames.has(cmd.name) &&
					!vscodeOnlyNames.has(cmd.name),
			);
	...
	private async setSessionTitleFromFirstExchange(
	...
	private async unstable_listSessions_internal(
		params: acp.ListSessionsRequest,
	): Promise<acp.ListSessionsResponse> {
		const persistedSessions = listLatestConversationHistoryItems(
			params.cwd,
			this.options.cwd,
		).map((historyItem) =>
		...
		const persistedSessionIds = new Set(
			persistedSessions.map((session) => session.sessionId),
		);
		...
		return {
			sessions: [...persistedSessions, ...activeOnlySessions].sort(
				(left, right) =>
					(right.updatedAt ?? "").localeCompare(left.updatedAt ?? ""),
			),
		};
	...
	async checkpointRestore(
		...
		await pWaitFor(() => controller.task?.taskState.isInitialized === true, {
			...
			Logger.error(
				"[DiracAgent.checkpointRestore] Failed to wait for task initialization:",
				error,
			);
			throw error;
		});
