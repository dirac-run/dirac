...
export class TaskMessageBridge {
    ...
    subscribeToTaskMessages(
        ...
        const onDiracMessagesChanged = (change: DiracMessageChange) => {
            this.handleDiracMessagesChanged(sessionId, sessionState, change, resolvePrompt, promptResolved).catch(
                (error) => this.handleUnhandledHandlerError(sessionId, promptResolved, resolvePrompt, error),
            )
        }
    ...
    private async handlePermissionRequest(
        ...
        const interactionType: "tool" | "followup" =
            message.content.type === DiracMessageType.CARD && message.content.card.requireApproval
                ? "tool"
                : "followup"
    ...
    private async processMessageWithDelta(
        ...
        if (
            ...
            const sessionUpdate: "agent_message_chunk" | "agent_thought_chunk" =
                message.content.isReasoning ? "agent_thought_chunk" : "agent_message_chunk"
