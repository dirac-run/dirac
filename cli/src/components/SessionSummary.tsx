        ...
        <Box borderColor="gray" borderStyle="single" flexDirection="column" paddingX={1} width={width}>
            ...
            <Box marginBottom={1}>
                ...
                <Text>
                    {(() => {
                        const barWidth = 10
                        const apiRatio = agentActiveMs > 0 ? stats.apiTimeMs / agentActiveMs : 0
                        const toolRatio = agentActiveMs > 0 ? stats.toolTimeMs / agentActiveMs : 0
                        const apiBlocks = Math.round(apiRatio * barWidth)
                        const toolBlocks = Math.round(toolRatio * barWidth)
                        const idleBlocks = barWidth - apiBlocks - toolBlocks
                        return (
                            <Text>
                                <Text color="cyan">{"█".repeat(apiBlocks)}</Text>
                                <Text color="yellow">{"█".repeat(Math.max(0, toolBlocks))}</Text>
                                <Text color="gray">{"░".repeat(Math.max(0, idleBlocks))}</Text>
                            </Text>
                        )
                    })()}
            ...
            {/* Tips */}
            <Box marginTop={1}>
                <Text color="gray" dimColor>Tip: Use Tab to switch between Plan and Act modes</Text>
            </Box>
        </Box>
    )
}
