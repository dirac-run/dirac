...
type SplitDisplayRow =
    | { type: "row"; oldLine?: DiffLine; newLine?: DiffLine }
...
function colorsForType(type: "add" | "remove" | "context" | undefined): { bgColor?: string; fgColor?: string; dimColor?: boolean } {
...
const SplitDiffRow: React.FC<{
    ...
    return (
        <Box flexDirection="row" width="100%">
            ...
            <Text color="gray" dimColor>{SPLIT_SEPARATOR}</Text>
                ...
                <React.Fragment key={blockIdx}>
                    ...
                    {renderSplit
                        ? splitBlocks[blockIdx].map((row, lineIdx) => (
                            <SplitDiffRow gutterWidth={gutterWidth} key={`${blockIdx}-${lineIdx}`} row={row} width={renderWidth} />
                        ))
                        : displayLines.map((item, lineIdx) =>
                            item.type === "collapsed" ? (
                                <UnifiedCollapsedRow count={item.count} gutterWidth={gutterWidth} key={`${blockIdx}-${lineIdx}`} />
                            ) : (
                                <UnifiedDiffLineRow gutterWidth={gutterWidth} key={`${blockIdx}-${lineIdx}`} line={item.line} />
                            ),
                        )}
                </React.Fragment>
            ))}
