...
function formatCardMessage(message: DiracMessage, prefix: string, verbose: boolean): string {
    ...
    const statusIndicator = card.status === CardStatus.SUCCESS ? style.success("✓ ") : card.status === CardStatus.ERROR ? style.error("✕ ") : ""
    const elapsed = card.startTime && card.endTime
        ? ` · ${((card.endTime - card.startTime) / 1000).toFixed(1)}s`
        : ""
...
function formatApiStatusMessage(message: DiracMessage, prefix: string, verbose: boolean): string {
    ...
    const tokensStr =
        status.tokensIn !== undefined
            ? `Tokens: ${status.tokensIn.toLocaleString()} in, ${status.tokensOut?.toLocaleString()} out${status.reasoningTokens ? ` (+${status.reasoningTokens.toLocaleString()} thinking)` : ""
            }`
            : ""
