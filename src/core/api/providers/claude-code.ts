...
export class ClaudeCodeHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: DiracStorageMessage[]): ApiStream {
		...
		for await (const chunk of claudeProcess) {
			if (typeof chunk === "string") { yield { type: "text", text: chunk }; continue }
			if (chunk.type === "system" && chunk.subtype === "init") { isPaidUsage = chunk.apiKeySource !== "none"; continue }
	...
	private *yieldAssistantContent(contentBlocks: any[]): Generator<any> {
		for (const content of contentBlocks) {
			switch (content.type) {
				case "text": yield { type: "text", text: content.text }; break
				case "thinking": yield { type: "reasoning", reasoning: content.thinking || "" }; break
				case "redacted_thinking": yield { type: "reasoning", reasoning: "[Redacted thinking block]" }; break
				case "tool_use": yield { type: "tool_calls", tool_call: { call_id: content.id, function: { id: content.id, name: content.name, arguments: JSON.stringify(content.input) } } }; break
			}
		}
	}
