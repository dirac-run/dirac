...
namespace Bedrock {
	 ...
	 */
	export function formatMessagesForConverseAPI(messages: DiracStorageMessage[]): BedrockMessage[] {
...
export class SapAiCoreHandler implements ApiHandler {
	...
	private async *createMessageWithOrchestration(systemPrompt: string, messages: DiracStorageMessage[]): ApiStream {
		try {
			...
			// sent directly to the LLM with the Dirac system prompt without validation errors.
	...
	private async *createMessageWithDeployments(systemPrompt: string, messages: DiracStorageMessage[]): ApiStream {
		const token = await this.getToken()
		const externalHeaders = buildExternalBasicHeaders()
		const headers = {
			...externalHeaders,
	...
	createUserReadableRequest(
		...
		return {
			...
			system: "(see SYSTEM_PROMPT in src/core/Dirac.ts)",
			messages: [{ conversation_history: "..." }, { role: "user", content: userContent }],
			tools: "(see tools in src/core/Dirac.ts)",
