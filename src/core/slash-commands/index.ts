...
export async function parseSlashCommands(
    ...
    const SUPPORTED_DEFAULT_COMMANDS = ["newtask", "smol", "compact", "newrule", "reportbug", "permissions", "askDirac", "reloadtools"]
        ...
        if (tagMatch) {
            ...
            if (SUPPORTED_DEFAULT_COMMANDS.includes(commandName)) {
                ...
                if (commandName === "reloadtools") {
                    return { processedText: "", needsDiracrulesFileCheck: false, isDirectResponse: true, directResponseText: "__RELOAD_TOOLS__" }
                }
