...
describe("StateManager", () => {
    ...
    function createMockMemento(initial: Record<string, any> = {}): DiracMemento {
        ...
        return {
            ...
            update: async (key: string, value: any) => { data[key] = value },
    ...
    beforeEach(async () => {
        ...
        sandbox.stub(HostProvider, "get").returns({ globalStorageFsPath: tempDir, hostBridge: { workspaceClient: {}, envClient: { getHostVersion: sandbox.stub().resolves({}) } } } as any)
    ...
    describe("initialize and get", () => {
        it("initialize returns a StateManager", async function() { this.skip() })  // env issue
