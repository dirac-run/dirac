...
export class LifecycleManager {
    ...
    public async initializeCheckpoints(isFirstRequest: boolean): Promise<void> {
        ...
        if (!this.dependencies.taskState.checkpointManagerErrorMessage) {
            ...
            const lastCheckpointMessageIndex = findLastIndex(
                messages,
                (m) => m.content.type === DiracMessageType.CHECKPOINT,
            )
    ...
    public async resumeTaskFromHistory() {
        ...
        const lastRelevantMessageIndex = findLastIndex(
            savedDiracMessages,
            (m) => !(m.content.type === "card" && (m.content.card.header === "Resume Task" || m.content.card.header === "Resume Completed Task")),
        )
        ...
        const lastDiracMessage = this.dependencies.messageStateHandler
            ...
            .slice()
            .reverse()
            .find((m) => !(m.content.type === "card" && (m.content.card.header === "Resume Task" || m.content.card.header === "Resume Completed Task")))
        ...
        await pWaitFor(
            () => this.dependencies.taskState.askResponse !== undefined || this.dependencies.taskState.abort,
            { interval: 100 }
        )
