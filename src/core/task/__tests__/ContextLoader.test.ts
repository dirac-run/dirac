...
describe("ContextLoader (characterization)", () => {
    ...
    function makeDependencies(overrides: Partial<ContextLoaderDependencies> = {}): ContextLoaderDependencies {
        return {
            ulid: "test-ulid",
            stateManager: { getGlobalSettingsKey: sandbox.stub().returns({}), getWorkspaceStateKey: sandbox.stub().returns({}) } as any,
            cwd: tempDir,
    ...
    function stubResolveWorkspacePassthrough() {
        ...
        workspaceResolverModule.resolveWorkspacePath = (_ctx: any, relPath: string) => path.isAbsolute(relPath) ? relPath : path.join(tempDir, relPath)
    ...
    describe("loadContext - content block processing", () => {
        ...
        it("processes text blocks wrapped in <task> tag", async () => {
            stubParseMentions("parsed-text"), stubParseSlashCommands({ processedText: "enriched", needsDiracrulesFileCheck: false }), stubSkills(), stubWorkflows()
            const loader = new ContextLoader(makeDependencies())
        ...
        it("processes text blocks wrapped in <feedback> tag", async () => {
            stubParseMentions("p"), stubParseSlashCommands({ processedText: "fb-enriched", needsDiracrulesFileCheck: false }), stubSkills(), stubWorkflows()
            const loader = new ContextLoader(makeDependencies())
        ...
        it("processes text blocks wrapped in <answer> tag", async () => {
            stubParseMentions("p"), stubParseSlashCommands({ processedText: "ans", needsDiracrulesFileCheck: false }), stubSkills(), stubWorkflows()
            const loader = new ContextLoader(makeDependencies())
        ...
        it("processes text blocks wrapped in <user_message> tag", async () => {
            stubParseMentions("p"), stubParseSlashCommands({ processedText: "um", needsDiracrulesFileCheck: false }), stubSkills(), stubWorkflows()
            const loader = new ContextLoader(makeDependencies())
        ...
        it("filters out skills disabled by global toggles", async () => {
            ...
            stubSkills(skills)
            const sm: any = { getGlobalSettingsKey: sandbox.stub().returns({ "/off": false }), getWorkspaceStateKey: sandbox.stub().returns({}) }
        ...
        it("filters out skills disabled by local toggles", async () => {
            ...
            stubSkills(skills)
            const sm: any = { getGlobalSettingsKey: sandbox.stub().returns({}), getWorkspaceStateKey: sandbox.stub().returns({ "/off": false }) }
            const deps = makeDependencies({ stateManager: sm })
            const loader = new ContextLoader(deps)
            const result = await loader.loadContext([{ type: "text", text: "<task>x</task>" }], false, false)
            result[3].should.have.length(1)
    ...
    describe("loadContext - tool_result content handling", () => {
        ...
        it("skips string tool_result content that looks like read_file output ([File Hash:])", async () => {
            stubParseMentions("p"), stubParseSlashCommands({ processedText: "ENRICHED", needsDiracrulesFileCheck: false }), stubSkills(), stubWorkflows()
            const loader = new ContextLoader(makeDependencies())
        ...
        it("processes array tool_result text blocks containing USER_CONTENT_TAGS", async () => {
            stubParseMentions("p"), stubParseSlashCommands({ processedText: "ARR-ENRICHED", needsDiracrulesFileCheck: false }), stubSkills(), stubWorkflows()
            const loader = new ContextLoader(makeDependencies())
        ...
        it("skips array tool_result text blocks that look like read_file output", async () => {
            stubParseMentions("p"), stubParseSlashCommands({ processedText: "X", needsDiracrulesFileCheck: false }), stubSkills(), stubWorkflows()
            const loader = new ContextLoader(makeDependencies())
    ...
    describe("loadContext - diracrules file check", () => {
        it("does not check diracrules when needsDiracrulesFileCheck is false", async () => {
            stubParseMentions("p"), stubParseSlashCommands({ processedText: "x", needsDiracrulesFileCheck: false }), stubSkills(), stubWorkflows()
            let called = false
            ruleHelpersModule.ensureLocalDiracDirExists = async () => { called = true; return false }
            const loader = new ContextLoader(makeDependencies())
            const result = await loader.loadContext([{ type: "text", text: "<task>x</task>" }], false, false)
        ...
        it("checks diracrules when needsDiracrulesFileCheck is true", async () => {
            stubParseMentions("p"), stubParseSlashCommands({ processedText: "x", needsDiracrulesFileCheck: true }), stubSkills(), stubWorkflows()
    ...
    describe("loadContext - direct response (/reloadtools)", () => {
        it("returns isDirectResponse and directResponseText for /reloadtools", async () => {
            stubParseMentions("p")
            stubParseSlashCommands({ processedText: "", needsDiracrulesFileCheck: false, isDirectResponse: true, directResponseText: "__RELOAD_TOOLS__" })
            ...
            toolRegistryModule.ToolRegistry.getInstance = () => ({
                getAllTools: () => [{ id: "t1", source: "builtin" }, { id: "t2", source: "user" }],
    ...
    describe("loadContext - includePathContext gating", () => {
        it("skips path/symbol context extraction on subsequent turns (includeFileDetails=true means includePathContext=true on first turn only)", async () => {
            ...
            stubParseMentions("p"), stubParseSlashCommands({ processedText: "JUST-TEXT", needsDiracrulesFileCheck: false }), stubSkills(), stubWorkflows()
            let extractCalled = false
            ruleConditionalsModule.extractSymbolLikeStrings = () => { extractCalled = true; return [] }
            const loader = new ContextLoader(makeDependencies())
            const result = await loader.loadContext([{ type: "text", text: "<task>x</task>" }], false, false)
        ...
        it("runs path/symbol context extraction when includeFileDetails=true", async () => {
            stubParseMentions("p"), stubParseSlashCommands({ processedText: "WITH-CTX", needsDiracrulesFileCheck: false }), stubSkills(), stubWorkflows()
    ...
    describe("extractContext - path & symbol detection (via loadContext with includeFileDetails=true)", () => {
        it("detects existing file paths and enriches with skeleton", async () => {
            stubParseMentions("p"), stubParseSlashCommands({ processedText: "CTX", needsDiracrulesFileCheck: false }), stubSkills(), stubWorkflows()
            stubResolveWorkspacePassthrough()
            ruleConditionalsModule.extractSymbolLikeStrings = () => []
        ...
        it("limits directory lists to 3 directories", async () => {
            ...
            astAnchorModule.ASTAnchorBridge.getFileSkeleton = async () => null
            const loader = new ContextLoader(makeDependencies())
            const result = await loader.loadContext([{ type: "text", text: `<task>see ${dirs.map(d => d + "/").join(" ")} here</task>` }], true, false)
        ...
        it("does not detect paths inside code fences", async () => {
            ...
            ruleConditionalsModule.extractSymbolLikeStrings = (t: string) => { symbolCalled = true; return [] }
            const fileName = "fenced.ts"
            await fs.writeFile(path.join(tempDir, fileName), "x")
            astAnchorModule.ASTAnchorBridge.getFileSkeleton = async () => null
            const loader = new ContextLoader(makeDependencies())
            const result = await loader.loadContext([{ type: "text", text: `<task>\`\`\`\n${fileName}\n\`\`\`\nhere</task>` }], true, false)
        ...
        it("does not detect URLs as paths", async () => {
            ...
            astAnchorModule.ASTAnchorBridge.getFileSkeleton = async () => null
            const loader = new ContextLoader(makeDependencies())
            const result = await loader.loadContext([{ type: "text", text: `<task>see https://example.com/foo.ts here</task>` }], true, false)
            ;(result[0][0] as any).text.should.not.containEql("file_skeleton")
        })
        ...
        it("deduplicates repeated file paths", async () => {
            ...
            astAnchorModule.ASTAnchorBridge.getFileSkeleton = async () => "SKEL"
            const loader = new ContextLoader(makeDependencies())
            const result = await loader.loadContext([{ type: "text", text: `<task>${fileName} and ${fileName} here</task>` }], true, false)
            const text = (result[0][0] as any).text as string
    ...
    describe("extractContext - symbol enrichment", () => {
        ...
        it("skips symbol enrichment when more than MAX_AUTO_SYMBOL_MATCHES (3) symbols detected", async () => {
            ...
            symbolIndexModule.SymbolIndexService.getInstance = () => ({
                getProjectRoot: () => tempDir,
                getDefinitions: () => { defsCalled = true; return [] },
                getReferences: () => [],
            }) as any
            try {
                const loader = new ContextLoader(makeDependencies())
    ...
    describe("loadContext - multiple content blocks", () => {
        it("processes multiple text blocks in parallel", async () => {
            stubParseMentions("p"), stubParseSlashCommands({ processedText: "E", needsDiracrulesFileCheck: false }), stubSkills(), stubWorkflows()
            const loader = new ContextLoader(makeDependencies())
