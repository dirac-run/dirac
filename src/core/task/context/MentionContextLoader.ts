...
export class MentionContextLoader {
    constructor(private dependencies: ContextLoaderDependencies, private fileContextLoader: FileContextLoader) { }

    // Parse mentions and slash commands, then optionally enrich with file/symbol context
    async enrichContext(
        text: string, cwd: string, localWorkflowToggles: any, globalWorkflowToggles: any,
        ulid: string, providerInfo: any, includePathContext: boolean, availableSkills: SkillMetadata[],
        ...
        const { processedText, needsDiracrulesFileCheck, isDirectResponse, directResponseText } = await parseSlashCommands(
            parsedText, localWorkflowToggles, globalWorkflowToggles, ulid, providerInfo, availableSkills,
            this.dependencies.commandPermissionController, this.dependencies.extensionPath, this.dependencies.sourceDir,
        )
        ...
        if (!includePathContext) return { enrichedText: processedText, needsDiracrulesFileCheck, isDirectResponse, directResponseText }
    ...
    private get urlContentFetcher() { return this.dependencies.urlContentFetcher }
    private get fileContextTracker() { return this.dependencies.fileContextTracker }
    private get workspaceManager() { return this.dependencies.workspaceManager }
}
