...
export class AskFollowupQuestionTool implements IDiracTool {
    ...
    async processCall(args: any, env: IToolEnvironment): Promise<any> {
        ...
        if (env.config.yoloModeToggled) {
            await env.ui.upsertText(`[YOLO MODE] Auto-responding to question: "${question.substring(0, 100)}${question.length > 100 ? "..." : ""}"`)
        ...
        if (optionsRaw && text && options.includes(text)) {
            ...
            const lastFollowupMessageIndex = findLastIndex(history, (m: any) => m.content.type === DiracMessageType.CARD && m.content.card.header === "Follow-up Question")
