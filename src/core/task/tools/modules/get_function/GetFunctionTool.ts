...
export class GetFunctionTool implements IDiracTool<GetFunctionArgs> {
	...
	async processCall(args: GetFunctionArgs, env: IToolEnvironment): Promise<any> {
		...
		const functionNames = Array.isArray(args.function_names)
			? args.function_names
			: args.function_names
				? [args.function_names]
				: []
	...
	private async reportMissingParameters(paths: string[], env: IToolEnvironment): Promise<string> {
		...
		if (!env.config.isSubagentExecution) {
			await env.ui.upsertText(
				`Dirac tried to use ${DiracDefaultTool.GET_FUNCTION} without providing a value for '${paramName}'. Retrying...`,
			)
		}
