...
export class RenameSymbolTool implements IDiracTool<RenameSymbolArgs> {
	...
	private async requestApproval(
		...
		const result = await env.interaction.askPermission(
			`Rename symbol '${existing_symbol}' to '${new_symbol}' in ${fileCount} file(s) (${occurrenceCount} occurrences)?`
		)
		if (result.action === DiracAskResponse.MESSAGE) {
			...
			if (cards) {
				for (const card of cards) {
					await card.update({ status: CardStatus.SKIPPED, body: `↩ Skipped — user sent a message instead` })
				}
			}
