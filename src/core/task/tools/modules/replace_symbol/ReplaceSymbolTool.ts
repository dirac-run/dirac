...
export class ReplaceSymbolTool implements IDiracTool<ReplaceSymbolArgs> {
	...
	async processCall(args: ReplaceSymbolArgs, env: IToolEnvironment): Promise<any> {
		...
		if (replacements.length === 0) {
			return this.reportMissingParameters(env)
		}
		...
		try {
			...
			if (shouldAutoApprove) {
				...
				const changeSummary = replacements.map((r) => `- \`${r.symbol}\` → new text in ${r.path}`).join("\n")
			...
			if (!approved) {
				return this.cancelReplacementWithUserReason(reason, Array.from(cards.values()), env)
			}
			...
			return this.finalizeCardsWithError(error, Array.from(cards.values()), env)
		}
	}
	...
	private async reportMissingParameters(env: IToolEnvironment): Promise<string> {
	...
	private async finalizeCardsWithError(error: any, cards: any[], env: IToolEnvironment): Promise<string> {
		await env.editor.hideReview()
		const mistakeCount = env.orchestration.getTaskState("consecutiveMistakeCount") + 1
		env.orchestration.setTaskState("consecutiveMistakeCount", mistakeCount)
		for (const card of cards) {
