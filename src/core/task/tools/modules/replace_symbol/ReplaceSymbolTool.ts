		...
		try {
			...
			if (shouldAutoApprove) {
				...
				const result = await env.interaction.askPermission(
					`Replace ${replacements.length} occurrence(s) of ${symbolSummary} in ${fileBatches.size} file(s)?`
				)
				...
				await result.card.finalize(result.approved ? CardStatus.SUCCESS : CardStatus.CANCELLED)
