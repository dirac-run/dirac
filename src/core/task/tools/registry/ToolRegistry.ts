...
export class ToolRegistry {
    ...
    getEnabledSpecs(context: SystemPromptContext): DiracToolSpec[] {
        return this.getEnabledTools()
            .map((t) => t.spec)
            .filter(
                (spec) => !spec.contextRequirements || spec.contextRequirements(context),
            )
    }
    ...
    private findToolByIdOrName(toolName: string): DiscoveredTool | undefined {
        return this.getAllTools().find(
            (tool) => tool.id === toolName || tool.name === toolName || tool.spec.name === toolName,
        )
    }
