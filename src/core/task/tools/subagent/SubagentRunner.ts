...
export class SubagentRunner {
    ...
    async run(
        ...
        try {
            ...
            const useNativeToolCalls = requestSnapshot.nativeTools.length > 0
...
${partialResult}`
    ...
    private buildSubagentRequestSnapshot(context: SystemPromptContext): ToolRequestSnapshot {
        ...
        const promptVisibleSpecs = DiracToolSet
            .withDynamicSubagentToolSpecs(contextFilteredSpecs, context)
            .filter((spec) => spec.id !== DiracDefaultTool.USE_SUBAGENTS || this.allowedTools.includes(spec.name))
    ...
    private buildSubagentCoordinator(enabledTools: DiscoveredTool[]): ToolExecutorCoordinator {
        ...
        for (const tool of enabledTools) {
            coordinator.registerModularTool(tool.factory(this.baseConfig))
        }
        return coordinator
    }

    private isDiscoveredToolAllowed(tool: DiscoveredTool): boolean {
    ...
    private createSubagentTaskConfig(state: TaskState, coordinator: ToolExecutorCoordinator): TaskConfig {
        ...
        const subagentContext = new DiracContext(
            `${this.baseConfig.taskId}-subagent`,
            this.baseConfig.services.stateManager,
        )
