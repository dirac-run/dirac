...
describe("Telemetry system is abstracted and can easily switch between providers", () => {
	...
	describe("Telemetry Service", () => {
		it("should include correct metadata with telemetry events", async () => {
			...
			telemetryService.captureToolUsage("task-456", "write_to_file", TEST_MODEL_IDS.OPENAI, "openai", false, true)
	...
	describe("No-Op Provider", () => {
		it("should create No-Op provider and handle all operations safely", async () => {
			...
			noOpTelemetryService.captureModelSelected(TEST_MODEL_IDS.OPENAI, "openai", "task-789")
			noOpTelemetryService.captureToolUsage("task-789", "write_to_file", TEST_MODEL_IDS.OPENAI, "openai", false, true)
