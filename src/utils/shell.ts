...
export function getAvailableTerminalProfiles(): TerminalProfile[] {
	...
	if (process.platform === "win32") {
		// Windows terminal profiles — only include shells that exist on this machine
		const windowsProfiles: TerminalProfile[] = [
			...
			},
			{
				id: "msys2-bash",
				name: "MSYS2 Bash",
				path: SHELL_PATHS.MSYS2_BASH,
				description: "MSYS2 Bash (bash.exe from MSYS2)",
			},
		]
		profiles.push(...windowsProfiles.filter((p) => !p.path || shellPathExists(p.path)))
...
/** Checks if a shell path exists on the filesystem (Windows only). */
...
export function getShell(): string {
	...
	if (process.platform === "win32") {
		// On Windows, try to detect an available shell before falling back to cmd.exe
		if (shellPathExists(SHELL_PATHS.POWERSHELL_7)) {
			return SHELL_PATHS.POWERSHELL_7
		}
		if (shellPathExists(SHELL_PATHS.GIT_BASH)) {
			return SHELL_PATHS.GIT_BASH
		}
		if (shellPathExists(SHELL_PATHS.MSYS2_BASH)) {
			return SHELL_PATHS.MSYS2_BASH
		}
		if (shellPathExists(SHELL_PATHS.POWERSHELL_LEGACY)) {
			return SHELL_PATHS.POWERSHELL_LEGACY
		}
		return SHELL_PATHS.CMD
	}
